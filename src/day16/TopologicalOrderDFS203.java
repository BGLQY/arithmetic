 ...
 */
public class TopologicalOrderDFS203 {
	...
	public static class MyComparator implements Comparator<Record> {
		...
		public int compare(Record o1, Record o2) {
			// compare two nodes
			// the one with the most nodes is the one with the most nodes
			return Long.compare(o2.allNodes,o1.allNodes);
		}
	}
	...
	private static Record queryDeep(DirectedGraphNode cur, HashMap<DirectedGraphNode, Record> order) {
		...
		for (DirectedGraphNode neighbor : cur.neighbors) {
			firstDeep += queryDeep(neighbor, order).allNodes;
		}
	...
	public static class Record {
		...
		public Record(DirectedGraphNode node, Long allNodes) {
			this.node = node;
